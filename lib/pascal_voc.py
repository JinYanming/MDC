...
class PascalVoc(Dataset):
    def __init__(self, cfg, mode='train', *args, **kwargs):
        ...
        if mode == 'train':
            ...
            trainclbsfile = osp.join(rootpath, 'ImageSets/Segmentation/trainclbs.txt')
        elif mode == 'val':
            ...
            testclbsfile = osp.join(rootpath, 'ImageSets/Segmentation/testclbs.txt')
        else: assert(False)
        jpgpth = osp.join(rootpath, 'JPEGImages')
        lbpth = osp.join(rootpath, 'SegmentationClass')
        print('dataset is prepared successfully!')
        with open(trainclbsfile, 'r') as fr:
    ...
    def __getitem__(self, idx):
        ...
        clabel = self.clbs[idx].strip()
        clabel = clabel.strip('[]')
        clabel = clabel.split(',')
        clabel = np.array(clabel)
        clabel = clabel.astype('float32')
        #print(type(clabel))
